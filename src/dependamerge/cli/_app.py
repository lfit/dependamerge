...
"""
The Typer application object, shared console, and CLI-wide constants.

Command modules import ``app`` from here and register themselves on it;
``dependamerge.cli`` imports those modules so every command is attached
by the time the console script resolves ``dependamerge.cli:app``.
"""
