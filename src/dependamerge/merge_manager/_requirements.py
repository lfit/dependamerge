...
"""
The pre-merge audit of a pull request's outstanding requirements.
"""
...
class _MergeRequirementsMixin(_MergeManagerBase):
    """Establishing what still stands between a PR and its merge."""
    ...
    async def _check_merge_requirements(
        ...
        verdict = await self._code_owner_review_verdict(pr_info, repo_owner, repo_name)
        if verdict is not None:
            return verdict

        if self.preview_mode:
            verdict = await self._predicted_merge_verdict(
        ...
        verdict = self._mergeable_state_verdict(pr_info, repo_owner, repo_name)
        if verdict is not None:
            return verdict

        return True, "All merge requirements appear to be met"

    async def _code_owner_review_verdict(
        self, pr_info: PullRequestInfo, repo_owner: str, repo_name: str
    ) -> tuple[bool, str] | None:
        """Decide the PR on its base branch's code-owner requirement.

        Returns ``None`` when code owner reviews are not required, or
        when the protection rules could not be read at all.
        """
        if not self._github_client:
            return None
    ...
    def _code_owner_bypass_verdict(
        ...
        """Answer whether the force level waives a code-owner review."""
    ...
    async def _predicted_merge_verdict(
        self, pr_info: PullRequestInfo, repo_owner: str, repo_name: str
    ) -> tuple[bool, str] | None:
        """Predictive merge probe.

        This is a *best-effort* dry-run verdict only: GitHub's
        mergeable_state can lag, and repository rulesets are invisible
        to it, so it must never gate the real merge. Run it only in
        preview mode to render the evaluation; the execution path is
        attempt-first and lets the actual merge response be
    ...
    async def _protection_bypass_verdict(
        ...
        """Confirm the token may actually bypass what the probe hit."""
        ...
        self.log.warning(
            f"⚠️ Bypassing branch protection check for {repo_owner}/{repo_name}#{pr_info.number}: {predicted_reason} (--force={self.force_level})"
        )
    ...
    def _mergeable_state_verdict(
        self, pr_info: PullRequestInfo, repo_owner: str, repo_name: str
    ) -> tuple[bool, str] | None:
        """Read the verdict off GitHub's mergeable state, where it says one."""
        ...
        if pr_info.mergeable_state == "blocked":
            return self._blocked_state_verdict(pr_info, repo_owner, repo_name)
    ...
    def _blocked_state_verdict(
        self, pr_info: PullRequestInfo, repo_owner: str, repo_name: str
    ) -> tuple[bool, str]:
        """Judge a ``blocked`` PR: approvable, forceable, or left to Step 5.5."""
